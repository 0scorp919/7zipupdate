...
**Поточна версія:** `7zip_manager.py` v1.4.1
...
- Шляхи з `.env` поруч зі скриптом (`devops/7zipupdate/.env`) або fallback на auto-detect
- Реєструється у системному PATH через `devops/pathupdate/fix_path.ps1` (має власний `.env` з `PATH_01..PATH_12`)
- **Не публікується** в GitHub (Windows ярлик `tags/7zip.lnk` з абсолютними шляхами)
...
1. **Перевірка системного PATH** — `apps/7zip/` у HKLM PATH, UAC → `devops/pathupdate/fix_path.ps1 -AutoClose`
...
devops/7zipupdate/
  ...
  .env                — конфігурація (gitignored, шляхи з auto-detect)
  .gitignore          — виключення для Git
  README.md           — ця документація

tags/
  7zip.lnk            — Windows ярлик (системний PATH → Win+R → 7zip)
...
> Резервна копія не створюється — 7-Zip Extra є CLI-інструментом без даних користувача.

## Конфігурація (.env)

Скрипт підтримує `.env` файл поруч із собою (`devops/7zipupdate/.env`):

```bash
# Приклад .env (не обов'язково, для портативності — без хардкодів)
SEVENZIP_DIR=%CAPSULE_ROOT%/apps/7zip
LOG_DIR=%CAPSULE_ROOT%/logs/7ziplog
...
**Відмінність від `tags/7zip.lnk`:**

- `tags/7zip.lnk` — Windows ярлик капсули, створюється динамічно
...
**GitHub репозиторій:** https://github.com/0scorp919/7zipupdate
...
- **v1.4.1** (2026-02-26) — Виправлено застарілий хардкод: `tags/7zip.bat` → `tags/7zip.lnk` (Windows ярлик).
...
- **v1.2** (2026-02-20) — Приведено до стандарту chromeupdate:
  ...
  - Стиль: cprint/log вирівняно з chrome_manager
  - Додано `.env.example` (пояснення чому `.env` не обов'язковий)
...
- **v1.1** (2026-02-20) — Стандарт менеджера капсули:
  ...
  - `cleanup_old_logs()` — ротація за розміром >10 MB
